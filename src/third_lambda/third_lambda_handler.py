...
from src.third_lambda.third_lambda_utils.get_inbuffer_parquet import (
...
def third_lambda_handler(event, context):
    ...
    This function:
        1) receives an event object from
            AWS S3 when the second lambda 
            function puts a dimension 
            table or the fact table in 
            a Parquet file in a BytesIO 
            buffer in the processed bucket.

        2) gets the Parquet file from the
            the processed bucket and from
            it creates a list of SQL queries
        ...
        3) connects to the data warehouse
            ...
            fact_sales_order table instead 
            add updated rows to the 
    ...
    Args:
        event: object generated by AWS
         S3 when the second lambda
         handler puts into the 
         processed bucket a BytesIO 
         buffer that contains a Parquet 
