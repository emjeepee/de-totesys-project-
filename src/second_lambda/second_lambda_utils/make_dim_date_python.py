...
def make_dim_date_python(
    ...
    Args:
        ...
        object for the date
        from which the date
        dimension table must
        begin.

        loop_range: a number
        of days from loop_date.
        Also the number of
    ...
    Returns:
        ...
        as a list of
        dictionaries that looks
         like this:
        [ {"date_id": start_date, "year": 2020, "month": 07, "day": 2 ... etc},
          {"date_id": -start_date+1day-, "year": 2020, "month": 07,
                                                "day": 3 ... etc},
          {"date_id": -start_date+2days-, "year": 2020, "month": 07,
                                                "day": 4 ... etc},
                        etc
        ],
        If loop_range is, eg,
        5000, there will be
        5000 member dictionaries
        in the list.
    ...
    for i in range(loop_range):
        row = {  # 19Aug25 --HAVE CHECKED THESE:
            ...
            "day_name": calendar.day_name[
                start_date.weekday()
            ],  # is str (eg 'Monday'). In warehouse, must be SQL VARCHAR
            "month_name": calendar.month_name[
                start_date.month
            ],  # is str (eg 'January'). In warehouse, must be SQL VARCHAR
            "quarter": (start_date.month - 1) // 3
            + 1,  # is int. In warehouse, must be SQL INT
        }
