...
def make_fake_so_table():
    ...
    # make consecutive 'created_at' values base_day + 1 day, base_day + 2 days, etc 
    ...
    # 'agreed_delivery_date':	'2025-08-20'  must be five days after 'created_at'
    # 'agreed_payment_date':  '2025-08-16',  must be three days after 'created_at'
    # 'unit_price' must be random string version of 2-digit float. There must be
    ...
    so_table =  [   {
            ...
            'last_updated':		base_day + (i * td_1_day) \
                                + td_1_day if i % 2 == 0 else  \
                                base_day + (i * td_1_day),
            ...
            'agreed_delivery_date':	(base_day + (i * td_1_day) + ((5 * td_1_day))).strftime('%Y-%m-%d'),
            'agreed_payment_date':  (base_day + (i * td_1_day) + ((3 * td_1_day))).strftime('%Y-%m-%d'),
