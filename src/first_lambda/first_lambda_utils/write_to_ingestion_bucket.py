...
def write_to_ingestion_bucket(data: list, bucket: str, file_location: str, s3_client: boto3.client):
    ...
    1. searches the ingestion 
        bucket for all tables 
        that are stored under a 
        key that begins with 
        file_location. This 
        function then gets the 
        most recent of those 
        tables (which is a 
        jsonified lists of
        dictionaries).
    2. converts the most recent 
        table to a python list 
        of dictionaries.
    3. replaces the appropriate 
        rows in the most recent 
        table.
    4. jsonifies the new updated 
        table.
    5. creates a timestamp string 
        that will be part of the 
        key under which to store 
        the jsonified table and 
        takes this form:
        '2025-05-28_15-45-03'.
    6. stores the new jsonified 
        table in the ingestion 
        bucket under a key that 
        looks like 
        'design/2025-05-28_15-45-03.json',
        where 'design' is the 
        value of arg file_location 
        and '2025-05-28_15-45-03' 
        is the timestamp.
    7. raises a RuntimeError if 
        either of functions 
    ...
    # Store the json list 
    # that represents the 
    # updated table in the 
    # bucket under the newly 
    # created key:
    save_updated_table_to_S3(
                updated_table_json, 
                s3_client, 
